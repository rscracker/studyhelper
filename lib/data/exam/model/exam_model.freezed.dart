// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target

part of 'exam_model.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more informations: https://github.com/rrousselGit/freezed#custom-getters-and-methods');

ExamModel _$ExamModelFromJson(Map<String, dynamic> json) {
  return _ExamModel.fromJson(json);
}

/// @nodoc
class _$ExamModelTearOff {
  const _$ExamModelTearOff();

  _ExamModel call(
      {required String examId,
      required String subject,
      required String examName,
      required DateTime date}) {
    return _ExamModel(
      examId: examId,
      subject: subject,
      examName: examName,
      date: date,
    );
  }

  ExamModel fromJson(Map<String, Object> json) {
    return ExamModel.fromJson(json);
  }
}

/// @nodoc
const $ExamModel = _$ExamModelTearOff();

/// @nodoc
mixin _$ExamModel {
  String get examId => throw _privateConstructorUsedError;
  String get subject => throw _privateConstructorUsedError;
  String get examName => throw _privateConstructorUsedError;
  DateTime get date => throw _privateConstructorUsedError;

  Map<String, dynamic> toJson() => throw _privateConstructorUsedError;
  @JsonKey(ignore: true)
  $ExamModelCopyWith<ExamModel> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $ExamModelCopyWith<$Res> {
  factory $ExamModelCopyWith(ExamModel value, $Res Function(ExamModel) then) =
      _$ExamModelCopyWithImpl<$Res>;
  $Res call({String examId, String subject, String examName, DateTime date});
}

/// @nodoc
class _$ExamModelCopyWithImpl<$Res> implements $ExamModelCopyWith<$Res> {
  _$ExamModelCopyWithImpl(this._value, this._then);

  final ExamModel _value;
  // ignore: unused_field
  final $Res Function(ExamModel) _then;

  @override
  $Res call({
    Object? examId = freezed,
    Object? subject = freezed,
    Object? examName = freezed,
    Object? date = freezed,
  }) {
    return _then(_value.copyWith(
      examId: examId == freezed
          ? _value.examId
          : examId // ignore: cast_nullable_to_non_nullable
              as String,
      subject: subject == freezed
          ? _value.subject
          : subject // ignore: cast_nullable_to_non_nullable
              as String,
      examName: examName == freezed
          ? _value.examName
          : examName // ignore: cast_nullable_to_non_nullable
              as String,
      date: date == freezed
          ? _value.date
          : date // ignore: cast_nullable_to_non_nullable
              as DateTime,
    ));
  }
}

/// @nodoc
abstract class _$ExamModelCopyWith<$Res> implements $ExamModelCopyWith<$Res> {
  factory _$ExamModelCopyWith(
          _ExamModel value, $Res Function(_ExamModel) then) =
      __$ExamModelCopyWithImpl<$Res>;
  @override
  $Res call({String examId, String subject, String examName, DateTime date});
}

/// @nodoc
class __$ExamModelCopyWithImpl<$Res> extends _$ExamModelCopyWithImpl<$Res>
    implements _$ExamModelCopyWith<$Res> {
  __$ExamModelCopyWithImpl(_ExamModel _value, $Res Function(_ExamModel) _then)
      : super(_value, (v) => _then(v as _ExamModel));

  @override
  _ExamModel get _value => super._value as _ExamModel;

  @override
  $Res call({
    Object? examId = freezed,
    Object? subject = freezed,
    Object? examName = freezed,
    Object? date = freezed,
  }) {
    return _then(_ExamModel(
      examId: examId == freezed
          ? _value.examId
          : examId // ignore: cast_nullable_to_non_nullable
              as String,
      subject: subject == freezed
          ? _value.subject
          : subject // ignore: cast_nullable_to_non_nullable
              as String,
      examName: examName == freezed
          ? _value.examName
          : examName // ignore: cast_nullable_to_non_nullable
              as String,
      date: date == freezed
          ? _value.date
          : date // ignore: cast_nullable_to_non_nullable
              as DateTime,
    ));
  }
}

/// @nodoc
@JsonSerializable()
class _$_ExamModel implements _ExamModel {
  _$_ExamModel(
      {required this.examId,
      required this.subject,
      required this.examName,
      required this.date});

  factory _$_ExamModel.fromJson(Map<String, dynamic> json) =>
      _$$_ExamModelFromJson(json);

  @override
  final String examId;
  @override
  final String subject;
  @override
  final String examName;
  @override
  final DateTime date;

  @override
  String toString() {
    return 'ExamModel(examId: $examId, subject: $subject, examName: $examName, date: $date)';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other is _ExamModel &&
            (identical(other.examId, examId) ||
                const DeepCollectionEquality().equals(other.examId, examId)) &&
            (identical(other.subject, subject) ||
                const DeepCollectionEquality()
                    .equals(other.subject, subject)) &&
            (identical(other.examName, examName) ||
                const DeepCollectionEquality()
                    .equals(other.examName, examName)) &&
            (identical(other.date, date) ||
                const DeepCollectionEquality().equals(other.date, date)));
  }

  @override
  int get hashCode =>
      runtimeType.hashCode ^
      const DeepCollectionEquality().hash(examId) ^
      const DeepCollectionEquality().hash(subject) ^
      const DeepCollectionEquality().hash(examName) ^
      const DeepCollectionEquality().hash(date);

  @JsonKey(ignore: true)
  @override
  _$ExamModelCopyWith<_ExamModel> get copyWith =>
      __$ExamModelCopyWithImpl<_ExamModel>(this, _$identity);

  @override
  Map<String, dynamic> toJson() {
    return _$$_ExamModelToJson(this);
  }
}

abstract class _ExamModel implements ExamModel {
  factory _ExamModel(
      {required String examId,
      required String subject,
      required String examName,
      required DateTime date}) = _$_ExamModel;

  factory _ExamModel.fromJson(Map<String, dynamic> json) =
      _$_ExamModel.fromJson;

  @override
  String get examId => throw _privateConstructorUsedError;
  @override
  String get subject => throw _privateConstructorUsedError;
  @override
  String get examName => throw _privateConstructorUsedError;
  @override
  DateTime get date => throw _privateConstructorUsedError;
  @override
  @JsonKey(ignore: true)
  _$ExamModelCopyWith<_ExamModel> get copyWith =>
      throw _privateConstructorUsedError;
}
